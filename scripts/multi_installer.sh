...
# Cirquity Multi-installer
# a one line clone-and-compile for cirquity:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/cirquity/cirquity/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/cirquity ]; then
            read -r -p "${1:-cirquity directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old cirquity directory" && echo
                    rm -rf "$PWD"/cirquity
                    ;;
                *)
                    _fail "cirquity directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir cirquity
        git clone -b master -q https://github.com/cirquity/cirquity cirquity   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd cirquity
    fi
}

_build_cirquity() {
    _note "Building cirquity from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/cirquity')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/cirquity')"
            ;;
    esac
